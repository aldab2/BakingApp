...
public class RecipeJsonUtils {
    ...
    public static ArrayList<Recipe> reviewJsonParse(String json) {
        json = dummyString;
        try {
            ...
            for(int i=0;i<results.length();i++){
                ...
                for( int j=0;j<jsonIngredients.length() || j< jsonSteps.length();j++){
                    if(j<jsonIngredients.length()){
                        ...
                        ingredient = jsonIngredient.getString("ingredient");
                    ...
                    if(j< jsonSteps.length()){
                        // Parse Steps
                        int stepId;
                        String shortDesc,description,vidURL,thubURL;
                        JSONObject jsonStep = jsonSteps.getJSONObject(j);
                        stepId = jsonStep.getInt("id");
                        shortDesc = jsonStep.getString("shortDescription");
                        description = jsonStep.getString("description");
                        vidURL= jsonStep.getString("videoURL");
