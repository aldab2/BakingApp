...
public class ClickingInredientTest {
    @Rule
   public ActivityTestRule<MainActivity> mActivitiyTestRule = new ActivityTestRule<MainActivity>(MainActivity.class);
    ...
    public void clickingIngredients_OpensTheCorrectFragment() throws InterruptedException {
        ...
        Espresso.onView(ViewMatchers.withId(R.id.tv_ingredient_quantity))
                .check(ViewAssertions.matches(ViewMatchers.withText(recipe.getIngredients().get(0).getQuantity()+"")));

        Espresso.onView(ViewMatchers.withId(R.id.tv_ingredient_measure))
                .check(ViewAssertions.matches(ViewMatchers.withText(recipe.getIngredients().get(0).getMeasure())));

        Espresso.onView(ViewMatchers.withId(R.id.tv_ingredient_ingredient))
                .check(ViewAssertions.matches(ViewMatchers.withText(recipe.getIngredients().get(0).getIngredient())));
    }
